...
let lookup_class : string -> class_env -> class_decl ea_result  = fun c_name c_env ->
  ...
  let rec new_env' fs =
    match fs with
     | [] -> EmptyEnv
     | h::t -> ExtendEnv(h,(RefVal (Store.new_ref g_store (NumVal 0))),new_env' t)
let rec new_env : string list -> env ea_result  = fun fs ->
  return (new_env' (List.rev fs))
...
and
  ...
  | NewObject(c_name,es) ->
    ...
    lookup_class c_name !g_class_env >>= fun tri ->
    (match tri with
    | (_, fields, methods) -> new_env fields >>= fun env -> 
      let m = List.assoc_opt "initialize" methods in 
      if m = None then return (ObjectVal(c_name, env)) else apply_method "initialize" (ObjectVal(c_name, env)) args (Option.get m) >>= fun _ -> return (ObjectVal(c_name, env)))

  | Send(e,m_name,es) ->
    eval_expr e >>=
    obj_of_objectVal >>= fun obj ->
    (match obj with
    | (name, ev) -> sequence (List.map eval_expr es) >>= fun args -> 
      let m = lookup_method name m_name !g_class_env in
      if m = None then error "Method not found" else apply_method m_name (ObjectVal(name, ev)) args (Option.get m))
  | Self ->
    eval_expr (Var "_self")
  | Super(m_name,es) ->
    sequence (List.map eval_expr es) >>= fun args ->
